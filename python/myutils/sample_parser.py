...
def make_filelist_fromPath(path, fileList):
  ...
  for line in lines:
    ...
    if line.find('.root') != -1: 
      ...
      make_filelist_fromPath(line,fileList)
...
#TEMP for LPC
          print samples_path
          if samples_path.find('root://cmseos.fnal.gov') != -1:
            tmp = samples_path.replace('root://cmseos.fnal.gov','')
            ls = os.popen("xrdfs root://cmseos.fnal.gov ls " + tmp)
          elif samples_path.find('.txt') != -1 or samples_path.find('.tex') != -1:
            ls = open(samples_path)
          else:
  ...
  #print 'will start the loop over the lines.'
    ...
    #print 'loop over the lines'
                ...
                if samples_path.find('.txt') != -1 or samples_path.find('.tex') != -1:
                  self.__fileslist.append(line.split()[1])
  ...
  #!! Else use the sample from  samples_nosplit.cfg (_config_entry).
      ...
      #fill the sample
            ...
            newsample.specialCut=config.get(sample,'specialCut')
