...
#############build list of files to run from folder#################
    ...
    #TEMP for LPC 
      ...
      use_grep = config.get('General','use_grep')
      FOLDER = pathIN+'/'+folderName
      if use_grep: FOLDER = pathIN
      ...
      if pathIN.startswith('root://cmseos.fnal.gov'):
        ...
        os.system('rm ' + tmp)
        if use_grep != 'True': util_funcs.findSubFolders(FOLDER, tmp)
        else: util_funcs.findSubFolders(FOLDER,tmp,True,folderName)
...
#        outputFile = "%s/%s/%s" %(pathOUT,folderName,filename.replace('.root','')+'_'+id_generator()+'.root')
    ...
    if multiprocess>1:
        ...
        p.close()
        p.join()
        print "Done with it multiprocessing"
